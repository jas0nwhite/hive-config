...
object NamingUtil extends Logging {
  ...
  val threeXP = s"""DA_5HT_NE${octaflowq}${fastq}"""
  ...
  def datasetInfoFromName(dataset: String): Option[InvitroDataset] = {
    dataset match {
      ...
      case threeX(dsDate, dsProtocol, null, null)              => Some(InvitroDataset(dsDate, "mixture", dsProtocol, "", ""))
      case threeX(dsDate, dsProtocol, probeName, null)         => Some(InvitroDataset(dsDate, "mixture", dsProtocol, probeName, ""))
      // scalastyle:on null
      ...
      case threeX(dsDate, dsProtocol, probeName, probeDate)    => Some(InvitroDataset(dsDate, "mixture", dsProtocol, probeName, probeDate))
